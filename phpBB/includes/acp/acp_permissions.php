...
class acp_permissions
{
	...
	*/
	function set_permissions($mode, $permission_type, &$auth_admin, &$user_id, &$group_id, $scripted = false)
	{
		...
		if (!is_array($scripted) && (!$auth->acl_get('a_' . str_replace('_', '', $permission_type) . 'auth') || !$auth->acl_get('a_auth' . $ug_type . 's')))
		{
		...
		if (!is_array($scripted) && (empty($_POST['setting']) || empty($_POST['setting'][$ug_id]) || empty($_POST['setting'][$ug_id][$forum_id]) || !is_array($_POST['setting'][$ug_id][$forum_id])))
		{
